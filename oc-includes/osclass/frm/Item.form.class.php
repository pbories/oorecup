    ...
    class ItemForm extends Form {
        ...
        static public function category_select($categories = null, $item = null, $default_item = null, $parent_selectable = false)
        {
            ...
            if(isset($default_item)) {
                ...
                echo '<option value="">' . __('Choisir une catégorie') . '</option>';
            }
        ...
        static public function country_select($countries = null, $item = null) {
            ...
            if( count($countries) >= 1 ) {
                ...
                parent::generic_select('countryId', $countries, 'pk_c_code', 's_name', __('Choisir un pays...'), (isset($item['fk_c_country_code'])) ? $item['fk_c_country_code'] : null);
                return true;
            } else {
        ...
        static public function region_select($regions = null, $item = null) {
            ...
            if( count($regions) >= 1 ) {
                ...
                parent::generic_select('regionId', $regions, 'pk_i_id', 's_name', __('Choisir une région...'), (isset($item['fk_i_region_id'])) ? $item['fk_i_region_id'] : null);
                return true;
            } else {
        ...
        static public function city_select($cities = null, $item = null) {
            ...
            if( count($cities) >= 1 ) {
                ...
                parent::generic_select('cityId', $cities, 'pk_i_id', 's_name', __('Choisir une ville...'), (isset($item['fk_i_city_id'])) ? $item['fk_i_city_id'] : null);
                return true;
            } else {
    ...
    $(document).ready(function(){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        if(length > 0) {
                            ...
                            result += '<option selected value=""><?php echo osc_esc_js(__("Choisir une région...")); ?></option>';
                            for(key in data) {
                        ...
                        $("#cityId").html('<option selected value=""><?php echo osc_esc_js(__("Choisir une ville...")); ?></option>');
                 ...
                 // add empty select
                 $("#region").before('<select name="regionId" id="regionId" ><option value=""><?php echo osc_esc_js(__("Choisir une région...")); ?></option></select>');
                 $("#region").remove();

                 $("#city").before('<select name="cityId" id="cityId" ><option value=""><?php echo osc_esc_js(__("Choisir une ville...")); ?></option></select>');
                 $("#city").remove();

                 if( $("#regionId").length > 0 ){
                     $("#regionId").html('<option value=""><?php echo osc_esc_js(__("Choisir une région...")); ?></option>');
                 } else {
                     $("#region").before('<select name="regionId" id="regionId" ><option value=""><?php echo osc_esc_js(__("Choisir une région...")); ?></option></select>');
                     $("#region").remove();
                 }
                 if( $("#cityId").length > 0 ){
                     $("#cityId").html('<option value=""><?php echo osc_esc_js(__("Choisir une ville...")); ?></option>');
                 } else {
                     $("#city").before('<select name="cityId" id="cityId" ><option value=""><?php echo osc_esc_js(__("Choisir une ville...")); ?></option></select>');
                     $("#city").remove();
                 }
            ...
            if(pk_c_code != '') {
                ...
                $.ajax({
                    ...
                    success: function(data){
                        var length = data.length;
                        if(length > 0) {
                            result += '<option selected value=""><?php echo osc_esc_js(__("Choisir une ville...")); ?></option>';
                            for(key in data) {
                                result += '<option value="' + data[key].pk_i_id + '">' + data[key].s_name + '</option>';
                            }
        ...
        $("form[name=item]").validate({
            ...
            messages: {
                ...
                <?php } ?>
                regionId: "<?php echo osc_esc_js(__("Choisir une région")); ?>.",
                cityId: "<?php echo osc_esc_js(__("Choisir une ville")); ?>.",
                cityArea: {
